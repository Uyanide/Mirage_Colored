export class Mirage_Colored {
    constructor(defaultArguments, innerCanvas, coverCanvas, outputCanvas, whiteCanvas, blackCanvas) {
        ...
        this._scale_c = defaultArguments.scale_c;
    ...
    _process = () => {
        ...
        if (this._is_colored) {
            if (!this._alphaCache || flag) {
                this._alphaCache = new Float64Array(this._innerDataCache.length >> 2);
                for (let i = 0; i < this._innerDataCache.length; i += 4) {
                    ...
                    this._alphaCache[i >> 2] = Math.max((1 + (((2048 | (dr + ((ir + cr) << 1))) * dr - (db + ((ir + cr) << 1) - 3068) * db + (dg << 12)) / (1020 * (dr - db) + 2349060))), 0);
                }
            }
            this._outputData = new Uint8ClampedArray(this._innerDataCache.length);
            for (let i = 0; i < this._innerDataCache.length; i += 4) {
                const a = this._alphaCache[i >> 2];
                this._outputData[i] = (this._innerDataCache[i] / a) * this._weight_i + (255 - (255 - this._coverDataCache[i]) / a) * (1 - this._weight_i);
                this._outputData[i + 1] = (this._innerDataCache[i + 1] / a) * this._weight_i + (255 - (255 - this._coverDataCache[i + 1]) / a) * (1 - this._weight_i);
                this._outputData[i + 2] = (this._innerDataCache[i + 2] / a) * this._weight_i + (255 - (255 - this._coverDataCache[i + 2]) / a) * (1 - this._weight_i);
